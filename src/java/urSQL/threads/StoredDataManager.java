...
public class StoredDataManager implements Callable {
    ...
    private final String _ExecutionPlan = logHandler.getInstance().getRootPath() + "/urSQL/DataBases/System_Catalog/Execution_Plan.txt";//"c:\\tmp/DataBases/System_Catalog/Execution_Plan.txt"; David
    private final String _Dir = logHandler.getInstance().getRootPath() + "/urSQL/DataBases/"; //"c:\\tmp/DataBases/"; david
    ...
    public String call() throws InterruptedException, ExecutionException, IOException, Exception {
        if (ThreadManager._DATA.get_ErrorFlag() == false) {
            executePlan();
        }
    ...
    private void process(String pCommand) throws Exception {
        ...
        if (CommandList.get(0).equals("CREATE_DB")) {
            System.out.println("creadndo db");
        ...
        if (CommandList.get(0).equals("SELECT")) {
            if (CommandList.get(1).equals("ALL")) {
                writeTable(TMP1);
            } else {
